...
class JsSpec extends FunSpec with ShouldMatchers {
  ...
  describe("to json of a boolean") {
    it("should serialize to json") {
      val b1 = true
      val b2 = false
      toJSON_n(b1) should equal("true")
      toJSON_n(b2) should equal("false")
    }
  }
  ...
  describe("to json of a List") {
    it("should serialize to json") {
      // List[Int]
      val l1 = List(1,2,3,4)
      toJSON_n(l1) should equal("[1,2,3,4]")

      // List[String]
      val l2 = List("a", "b", "c", "d")
      toJSON_n(l2) should equal("""["a","b","c","d"]""")

      // List[List[String]]
      val l3 = List(List("a", "b"), List("c", "d"), List("e", "f", "g"))
      toJSON_n(l3) should equal("""[["a","b"],["c","d"],["e","f","g"]]""")

      // List[Tuple2]
      val l4 = List((1, "a"), (2, "b"), (3, "c"))
      toJSON_n(l4) should equal("""[{1:"a"},{2:"b"},{3:"c"}]""")
    }
  }
  ...
  describe("to json of a Map") {
    it("should serialize to json") {
      ...
      toJSON_n(m1) should equal("""{"1":"a","2":"b","3":"c"}""")

      // Map Int -> Int
      val m2 = Map(1->1, 2->2, 3->3)
      toJSON_n(m2) should equal("""{"1":1,"2":2,"3":3}""")

      // Map Int -> List[Int]
      val m3 = Map(1->List(1,2,3), 2->List(4,5,6), 3->List(7,8,9))
      toJSON_n(m3) should equal("""{"1":[1,2,3],"2":[4,5,6],"3":[7,8,9]}""")

      // Map Int -> List[List[Int]]
      val m4 = Map(1->List(List(1,2),List(3)), 2->List(List(4),List(5,6)), 3->List(List(7,8,9)))
      toJSON_n(m4) should equal("""{"1":[[1,2],[3]],"2":[[4],[5,6]],"3":[[7,8,9]]}""")
    }
  }
  ...
  describe("to json of an object") {
    it("should serialize to json") {
      ...
      toJSON_n(s) should equal("""{"price":100,"item":"fridge","store":"kmart"}""")
      ...
      toJSON_n(c) should equal("""{"addresses":{"primary":{"zip":"80231","city":"Denver","street":"Tamarac Street"},"secondary":{"zip":"95054","city":"Cupertino","street":"Garden Street"}},"name":"debasish ghosh"}""")

      // object with an optional field
      val oa1 = AddressWithOptionalCity("Tamarac Street", Some("Denver"), "80231")
      toJSON_n(oa1) should equal("""{"zip":"80231","city":"Denver","street":"Tamarac Street"}""")
      val oa2 = AddressWithOptionalCity("Tamarac Street", None, "80231")
      toJSON_n(oa2) should equal("""{"zip":"80231","city":[],"street":"Tamarac Street"}""")
      ...
      toJSON_n(p) should equal("""{"addresses":[{"zip":"80231","city":"Denver","street":"Tamarac Street"},{"zip":"95054","city":"Cupertino","street":"Garden Street"}],"firstName":"debasish","lastName":"ghosh"}""")
      ...
      val e = new Employee(100,
                           ...
                           addresses,
                           Salary(4500, 245))
      toJSON_n(e) should equal("""{"sal":{"allowance":245,"basic":4500},"addresses":[{"zip":"94111","city":"San Francisco, CA","street":"10 Market Street"},{"zip":"98301","city":"Denver, CO","street":"3300 Tamarac Drive"}],"prevEmployer":"Circuit City","name":"Jason Alexander","id":100}""")

      val arr = ArrayTest(100, "debasish", Array("abc", "def", "ghi"))
      toJSON_n(arr) should equal("""{"addresses":["abc","def","ghi"],"name":"debasish","id":100}""")
    }
  }
  ...
  describe("from json of an object") {
    it("should de-serialize from json") {
      ...
      em.addresses should equal(e.addresses)
      em.sal should equal(e.sal)
    }
  }
